...
This project demonstrates the design and implementation of a Security Orchestration, Automation, and Response (SOAR) workflow integrated with an Endpoint Detection and Response (EDR) platform. The objective was to automate the incident response lifecycle by detecting malicious activity, notifying security analysts, incorporating analyst approval into the decision-making process, and automatically containing compromised endpoints.

Using LimaCharlie, Tines, Slack, and Email, I developed an automated workflow that detects execution of LaZagne, generates analyst notifications, collects analyst approval through an interactive workflow, and performs endpoint isolation only after approval is granted. This project demonstrates how automation can accelerate incident response while maintaining analyst oversight before executing high-impact containment actions.

---

# Architecture

The environment consists of a Windows endpoint monitored by LimaCharlie EDR. When malicious activity is detected, LimaCharlie forwards the detection to Tines through a webhook. Tines orchestrates the incident response workflow by notifying analysts through Slack and Email, requesting analyst approval, and executing endpoint isolation through LimaCharlie when approved. Following containment, the workflow validates the endpoint's isolation status and provides confirmation back to the analyst.

<img width="954" height="782" alt="29  Diagram" src="https://github.com/user-attachments/assets/bde6553f-e296-40a2-90a7-1409fdb18da3" />

---

# Skills Learned

- Designed and implemented a SOAR incident response workflow.
- Configured LimaCharlie EDR for endpoint monitoring and automated response.
- Developed and validated custom detection logic for credential dumping activity.
- Integrated LimaCharlie with Tines using webhook automation.
- Automated analyst notifications using Slack and Email.
- Implemented analyst approval workflows for security-sensitive response actions.
- Automated endpoint isolation through LimaCharlie.
- Validated automated containment actions using endpoint telemetry.
- Applied incident response and detection engineering principles.
- Documented an end-to-end SOAR workflow using professional security documentation.
...
| Tool                     | Purpose                                                                                  |
| ------------------------ | ---------------------------------------------------------------------------------------- |
| **LimaCharlie**          | Endpoint Detection and Response (EDR), detection rules, and automated endpoint isolation |
| **Tines**                | Security orchestration and workflow automation platform                                  |
| **Windows 11**           | Endpoint used to simulate malicious activity                                             |
| **LaZagne**              | Credential dumping tool used to simulate attacker behavior                               |
| **Slack**                | Analyst notification platform                                                            |
| **Email**                | Secondary analyst notification channel                                                   |
| **Vultr**                | Cloud-hosted Windows virtual machine                                                     |
| **Remote Desktop (RDP)** | Remote administration of the Windows endpoint                                            |
...
| ------------------------------- | ------------------------------------------------------------- |
| **LaZagne.exe**                 | Credential dumping utility executed during attack simulation  |
| **Credential Dumping Activity** | Simulated credential access detected by LimaCharlie           |
| **Custom Detection Rule**       | Detection engineered to identify LaZagne execution            |
| **LimaCharlie Detection Alert** | Security event generated after malicious activity             |
| **Endpoint Isolation Event**    | Automated containment action performed after analyst approval |
| **Slack Notification**          | Automated SOC alert generated by the SOAR workflow            |
| **Email Notification**          | Secondary analyst notification confirming detection           |
| **Tines Workflow Execution**    | Successful orchestration of automated incident response       |
...
| Impact            | Network Denial of Service *(Endpoint Isolation / Containment)* | T1498*       |
...
| ----- | ---------------------------------------------------------------------------------------------------------------------------------- |
| **1** | LimaCharlie detects execution of LaZagne on the Windows endpoint.                                                                  |
| **2** | Detection is forwarded to Tines through a webhook.                                                                                 |
| **3** | Tines sends automated notifications through Slack and Email.                                                                       |
| **4** | An analyst reviews the alert details and selects **Yes** or **No** within the approval workflow.                                   |
| **5** | If **No** is selected, the workflow terminates while notifying the SOC team that additional investigation is required.             |
| **6** | If **Yes** is selected, Tines instructs LimaCharlie to isolate the compromised endpoint.                                           |
| **7** | LimaCharlie performs endpoint isolation and blocks network communication.                                                          |
| **8** | Tines verifies the endpoint isolation status and sends confirmation to Slack, completing the automated incident response workflow. |
...
- Successfully implemented a complete SOAR workflow integrating LimaCharlie, Tines, Slack, and Email.
- Developed custom detection logic capable of identifying credential dumping activity.
- Automated analyst notifications across multiple communication channels.
- Implemented analyst approval before executing high-impact containment actions.
- Successfully automated endpoint isolation using LimaCharlie EDR.
- Validated end-to-end incident response from detection through containment verification.
- Demonstrated how security orchestration can reduce analyst response time while maintaining human oversight.

---

# Lessons Learned

This project strengthened my understanding of how modern SOC teams integrate EDR and SOAR platforms to automate incident response while maintaining analyst control over critical response actions. I gained hands-on experience designing workflows that combine detection, notification, analyst decision-making, and automated containment into a single repeatable process.

One of the most valuable lessons from this project was recognizing that effective security automation is not solely about eliminating manual effort—it is about enabling analysts to make informed decisions more quickly while ensuring response actions are executed consistently and reliably. By incorporating analyst approval before endpoint isolation, this project demonstrates a balanced approach that combines automation with human oversight, reflecting real-world incident response practices.
